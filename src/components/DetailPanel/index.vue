      ...
      <div v-if="status=='node-selected'" class="pannel" id="node_detailpannel">
        ...
        <div class="block-container">
          <el-row :gutter="10">
            ...
            <el-col :span="8">是否有算法</el-col>
            <el-col :span="16">
              <el-radio-group v-model="hasAlgorithm" @change="handleChange">
...
export default {
  ...
  methods: {
      ...
      eventBus.$on("afterAddPage", page => {
        ...
        eventBus.$on("nodeselectchange", item => {
          if (item.select === true && item.target.getType() === "node") {
            ...
            self.hasAlgorithm = item.target.hasState('setOK');
          } else {
    ...
    handleChange(val) {
      if(val) {
        this.item.setState('setOK', true)
      } else {
        this.item.setState('setOK', false)
      }
    }
  }
};
